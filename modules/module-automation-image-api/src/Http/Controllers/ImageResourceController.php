...
namespace Liberu\Modules\Automation\Image\Api\Http\Controllers;

use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;
use Illuminate\Routing\Controller;
use Liberu\Modules\Automation\Image\Actions\CreateImageResource;
use Liberu\Modules\Automation\Image\Actions\TransitionImageResource;
use Liberu\Modules\Automation\Image\Api\Support\ResourcePayload;
use Liberu\Modules\Automation\Image\Models\ImageResource;

final class ImageResourceController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        $resources = ImageResource::query()->forTeam($teamId)->latest()->paginate($pageSize, ['*'], 'page', $page);

        return response()->json(ResourcePayload::collection($resources));
    }

    public function store(Request $request, CreateImageResource $create): JsonResponse
    {
    ...
    public function show(Request $request, string $id): JsonResponse
    {
        ...
        return response()->json(['data' => ResourcePayload::one(ImageResource::query()->forTeam($teamId)->findOrFail($id))]);
    }
    ...
    public function update(Request $request, string $id): JsonResponse
    {
        ...
        $teamId = (string) $request->user()->currentTeam?->getKey();
        abort_if($teamId === '', 403);
        $resource = ImageResource::query()->forTeam($teamId)->findOrFail($id);
    ...
    public function destroy(Request $request, string $id): JsonResponse
    {
        $teamId = (string) $request->user()->currentTeam?->getKey();
        abort_if($teamId === '', 403);
        ImageResource::query()->forTeam($teamId)->findOrFail($id)->delete();

        return response()->json(status: 204);
    }

    public function transition(Request $request, string $id, TransitionImageResource $transition): JsonResponse
    {
