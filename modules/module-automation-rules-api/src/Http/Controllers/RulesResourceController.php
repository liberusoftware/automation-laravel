...
use Liberu\Modules\Automation\Rules\Actions\TransitionRulesResource;
use Liberu\Modules\Automation\Rules\Api\Support\ResourcePayload;
...
final class RulesResourceController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        $resources = RulesResource::query()->forTeam($teamId)->latest()->paginate($pageSize, ['*'], 'page', $page);
    ...
    public function show(Request $request, string $id): JsonResponse
    {
        ...
        return response()->json(['data' => ResourcePayload::one(RulesResource::query()->forTeam($teamId)->findOrFail($id))]);
    }
    ...
    public function update(Request $request, string $id): JsonResponse
    {
        ...
        $teamId = (string) $request->user()->currentTeam?->getKey();
        abort_if($teamId === '', 403);
        $resource = RulesResource::query()->forTeam($teamId)->findOrFail($id);
    ...
    public function destroy(Request $request, string $id): JsonResponse
    {
        $teamId = (string) $request->user()->currentTeam?->getKey();
        abort_if($teamId === '', 403);
        RulesResource::query()->forTeam($teamId)->findOrFail($id)->delete();

        return response()->json(status: 204);
    }

    public function transition(Request $request, string $id, TransitionRulesResource $transition): JsonResponse
    {
