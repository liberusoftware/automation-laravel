...
use Liberu\Modules\Automation\Evaluation\Actions\TransitionEvaluationResource;
use Liberu\Modules\Automation\Evaluation\Api\Support\ResourcePayload;
...
final class EvaluationResourceController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        $resources = EvaluationResource::query()->forTeam($teamId)->latest()->paginate($pageSize, ['*'], 'page', $page);
    ...
    public function show(Request $request, string $id): JsonResponse
    {
        ...
        return response()->json(['data' => ResourcePayload::one(EvaluationResource::query()->forTeam($teamId)->findOrFail($id))]);
    }
    ...
    public function update(Request $request, string $id): JsonResponse
    {
        ...
        $teamId = (string) $request->user()->currentTeam?->getKey();
        abort_if($teamId === '', 403);
        $resource = EvaluationResource::query()->forTeam($teamId)->findOrFail($id);
    ...
    public function destroy(Request $request, string $id): JsonResponse
    {
        $teamId = (string) $request->user()->currentTeam?->getKey();
        abort_if($teamId === '', 403);
        EvaluationResource::query()->forTeam($teamId)->findOrFail($id)->delete();

        return response()->json(status: 204);
    }

    public function transition(Request $request, string $id, TransitionEvaluationResource $transition): JsonResponse
    {
