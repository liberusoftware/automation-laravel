...
namespace Liberu\Modules\Automation\AiGateway\Api\Http\Controllers;

use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;
use Illuminate\Routing\Controller;
use Liberu\Modules\Automation\AiGateway\Actions\CreateAiGatewayResource;
use Liberu\Modules\Automation\AiGateway\Actions\TransitionAiGatewayResource;
use Liberu\Modules\Automation\AiGateway\Api\Support\ResourcePayload;
use Liberu\Modules\Automation\AiGateway\Models\AiGatewayResource;

final class AiGatewayResourceController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        $resources = AiGatewayResource::query()->forTeam($teamId)->latest()->paginate($pageSize, ['*'], 'page', $page);

        return response()->json(ResourcePayload::collection($resources));
    }

    public function store(Request $request, CreateAiGatewayResource $create): JsonResponse
    {
    ...
    public function show(Request $request, string $id): JsonResponse
    {
        ...
        return response()->json(['data' => ResourcePayload::one(AiGatewayResource::query()->forTeam($teamId)->findOrFail($id))]);
    }
    ...
    public function update(Request $request, string $id): JsonResponse
    {
        ...
        $teamId = (string) $request->user()->currentTeam?->getKey();
        abort_if($teamId === '', 403);
        $resource = AiGatewayResource::query()->forTeam($teamId)->findOrFail($id);
    ...
    public function destroy(Request $request, string $id): JsonResponse
    {
        $teamId = (string) $request->user()->currentTeam?->getKey();
        abort_if($teamId === '', 403);
        AiGatewayResource::query()->forTeam($teamId)->findOrFail($id)->delete();

        return response()->json(status: 204);
    }

    public function transition(Request $request, string $id, TransitionAiGatewayResource $transition): JsonResponse
    {
