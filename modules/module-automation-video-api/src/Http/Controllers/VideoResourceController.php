...
use Liberu\Modules\Automation\Video\Actions\TransitionVideoResource;
use Liberu\Modules\Automation\Video\Api\Support\ResourcePayload;
...
final class VideoResourceController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        $resources = VideoResource::query()->forTeam($teamId)->latest()->paginate($pageSize, ['*'], 'page', $page);
    ...
    public function show(Request $request, string $id): JsonResponse
    {
        ...
        return response()->json(['data' => ResourcePayload::one(VideoResource::query()->forTeam($teamId)->findOrFail($id))]);
    }
    ...
    public function update(Request $request, string $id): JsonResponse
    {
        ...
        $teamId = (string) $request->user()->currentTeam?->getKey();
        abort_if($teamId === '', 403);
        $resource = VideoResource::query()->forTeam($teamId)->findOrFail($id);
    ...
    public function destroy(Request $request, string $id): JsonResponse
    {
        $teamId = (string) $request->user()->currentTeam?->getKey();
        abort_if($teamId === '', 403);
        VideoResource::query()->forTeam($teamId)->findOrFail($id)->delete();

        return response()->json(status: 204);
    }

    public function transition(Request $request, string $id, TransitionVideoResource $transition): JsonResponse
    {
