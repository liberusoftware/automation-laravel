...
final readonly class VideoDelivery
{
    ...
    {
        if (filter_var($url, FILTER_VALIDATE_URL) === false || parse_url($url, PHP_URL_SCHEME) !== 'https' || parse_url($url, PHP_URL_USER) !== null || ! in_array($format, ['mp4', 'webm'], true) || $expiresAt <= time()) {
